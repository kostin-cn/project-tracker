  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <template v-if="isLoading">
      <div
        v-for="n in 3"
        :key="n"
        class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-xs flex flex-col justify-between animate-pulse"
      >
        <div>
          <!-- Шапка скелетона: Назва + Бейдж -->
          <div class="flex items-center justify-between gap-3 mb-4">
            <div class="h-6 bg-slate-200 dark:bg-slate-800 rounded-md w-1/2"></div>
            <div class="h-5 bg-slate-200 dark:bg-slate-800 rounded-full w-16 shrink-0"></div>
          </div>
          ...
          <!-- Дата створення -->
          <div class="h-3.5 bg-slate-200 dark:bg-slate-800 rounded-md w-1/3 mb-6"></div>
        </div>

        <div>
          <!-- Прогрес-бар скелетон -->
          <div class="space-y-2 mb-4">
            <div class="flex justify-between items-center">
              <div class="h-3.5 bg-slate-200 dark:bg-slate-800 rounded w-16"></div>
              <div class="h-3.5 bg-slate-200 dark:bg-slate-800 rounded w-10"></div>
            </div>
            <div class="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full"></div>
          </div>

          <!-- Кнопка скелетон -->
          <div class="w-full h-9 bg-slate-200 dark:bg-slate-800 rounded-lg"></div>
        </div>
      </div>
    </template>

    <!-- СТАН З ДАНИМИ (Реальні картки) -->
    <template v-else>
      <article
